...
Try {
	...
	[string]$appVendor = 'Zortrax'
	[string]$appName = 'Z-Suite'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		$exitCode = Execute-Process -Path "$dirFiles\Z-Suite_v2.4.0_Win.exe" -Parameters "/verysilent /suppressmsgboxes" -WindowStyle "Hidden" -PassThru
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		$exitCode = Execute-Process -Path "C:\Program Files\Zortrax\Z-Suite\unins000.exe" -Parameters "/silent" -WindowStyle "Hidden" -PassThru
        If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
